...
class CollaborativeFiltering(MatrixHandler):
    ...
    def get_top_k_similar(self, user_index):
        ...
        cos_similarity = (x @ self.X.T / U_norm).reshape(-1)
...
class Users(MatrixHandler):
    ...
    def __init__(self, n_users, n_items, list_alphas, list_betas, list_sizes):
        super().__init__(n_users, n_items)
        self.list_alphas = []
        self.list_betas = []
        self.list_sizes = []
        ...
        item_index = 0
        for alpha, beta, size in zip(list_alphas,
                                     list_betas,
                                     list_sizes):
            ...
            for _ in range(size):
                self.update_item_column(item_index, vals.pmf(x))
                item_index += 1
